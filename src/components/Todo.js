...
import { deleteTodo, editTodo, toggleComplete } from '../reducer';
    ...
    return (
        <div className="wrapper__box">
            <div className="wrapper__box--inner">
                <input className="wrapper__checkbox" type="checkbox" onClick={() => dispatch(toggleComplete(todo.id))} />
                <p className={`wrapper__task ${todo.isCompleted ? "completed" : "incompleted"}`}>{todo.task}</p>
            </div>
            <div className="wrapper__icons">
                <img className="wrapper__edit" src={edit} alt="edit" onClick={() => dispatch(editTodo(todo.id))} />
